...
SVMClassifier::SVMClassifier(string className) {
...
void SVMClassifier::trainModel(std::vector<std::vector<double>> &features, vector<double> &labels, parameter &param,int bias) {
...
void SVMClassifier::saveModel(string pathFile)
{
    save_model(pathFile.c_str(), modelLinearSVM);
}

void SVMClassifier::loadModel(string pathFile)
{
...
/*solver_type can be one of L2R_LR, L2R_L2LOSS_SVC_DUAL, L2R_L2LOSS_SVC, L2R_L1LOSS_SVC_DUAL, MCSVM_CS, L1R_L2LOSS_SVC, L1R_LR, L2R_LR_DUAL, L2R_L2LOSS_SVR, L2R_L2LOSS_SVR_DUAL, L2R_L1LOSS_SVR_DUAL.
    ...
    */
parameter SVMClassifier::initialiseParam(int solverTYPE, double C, double eps)
{
    parameter param;
...
double SVMClassifier::predictModel(vector<double> features)
{
...
vector<vector<double>> SVMClassifier::readFeatures(string filePath)
{
